...
for i in range(len(diste[300:])):
    if diste[300:][i]==0:
        cut=i+300-1
        print('diste Zero point is ', i+300)
        break
...
for i in range(len(newdistex[:5000])):
    if newdistex[:5000][i]==0:
        count+=1
...
sim2_unscaled = (sp.array(newdistex))+(sp.array(disteex))+nor(dispa,11.5*3.53-17,(11.5*3.53-17)**0.5,0.68,0,0) + nor(dispa,662,662*0.075,1.4,0,0)
...
for l in range(3000,5890):     # Check here for 10 factor
    n=int(sim2[l]*r) #
    # random seeds
    randseed=[]
    for i in range(0,n):
...
Ep = [int((11.5*10*3.53)-17), 6620]     
           
ampd=sp.array([0]*num)
#the number of points
for l in range(2):
    n=int(sim2[int(Ep[l])]*r) #
    # random seeds
    randseed=[]
    for i in range(0,n):
        randseed.append(random.uniform(0, 1))
    # noise
    #s=res*Ai[l]/2
    s=(1.26683723*(Ep[l])**0.5+1.92435425)*gain  # What are these?
    noise=-2**0.5*s*sps.erfinv(1-2*sp.array(randseed))
...
plt.plot(sp.linspace(0,520,105)/0.512,(ampd1),'o')
nK = 380
plt.errorbar(A[0:nK]*3.53-17,(cCs-cno)[0:nK],fmt='yo',yerr=sp.sqrt(np.abs((cCs-cno)[0:nK])))
plt.xlabel('Energy, KeV')
plt.ylabel('Frequency')
